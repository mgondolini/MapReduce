...
//  filtra le parole con lunghezza>0, map trasformazione chiave valore
    val rddMapDc = rddDcWords2.filter(_.length > 0).map(x => (x.substring(0,1), x.length))
    rddMapDc.collect()

//  aggregate by key permette di specificare il valore degli accumulatori
//  calcola per ogni tupla la somma di tutte le lunghezze e il conteggio delle parole
    val rddReduceDc = rddMapDc.aggregateByKey((0.0,0.0))((a,v)=>(a._1+v,a._2+1), (a1,a2)=>(a1._1+a2._1,a1._2+a2._2))
    rddReduceDc.collect()

//  ottiene la media facendo la divisione dei valori ottenuti
